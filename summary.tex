...
\begin{itemize}
    ...
    \item \textbf{Performance Profiling:} Added \texttt{--profile} flag to track CPU time, wall time, and disk usage for each stage of the matrix test, providing insights into system performance.
    \item \textbf{Reusable Worktree Optimization:} Optimized the matrix execution loop to use a persistent, reusable worktree with targeted \texttt{git reset} and \texttt{git clean} logic. This resulted in a \textbf{6x speedup} in version switching (from 1.5s to 0.24s per version).
